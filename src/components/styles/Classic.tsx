...
const BASE_STYLES = StyleSheet.create({
  ...
  rule: { height: 2.5, width: '100%', marginBottom: 10 },
  ...
  summary: { fontSize: 10, color: '#333', textAlign: 'center', marginBottom: 4 },
...
export const Classic: CvStyleComponent = ({ cv, backupString }) => {
    ...
    <Document keywords={backupString}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={[styles.name, { color: accent }]}>{cv.profile.fullName.toUpperCase()}</Text>
          <View style={[styles.rule, { backgroundColor: accent2 }]} />
          <Text style={styles.headline}>{cv.profile.headline}</Text>
          <ContactBlock cv={cv} style={styles.contact} lanesAlign="center" />
        </View>
