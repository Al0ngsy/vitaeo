...
try {
  ...
  const { CV_STYLES } = await server.ssrLoadModule(
  ...
  mkdirSync("docs/design-mockups/verify", { recursive: true });
  for (const style of CV_STYLES) {
    ...
    writeFileSync(`docs/design-mockups/verify/${style.id}.pdf`, buf);
